...
var _io = new ImportOptions(File(baseFolder + "projetos/esportes2023_confrontos_manual_nba.aep"));
...
for (var j = 1; j <= jogos.length; j++) {
  data = (jogos[j-1]['data'])
  hora = (jogos[j-1]['hora'])
  local = jogos[j-1]['local']
  TrocarTexto(comp_tela, "J" + j.toString() + "Dia", data);
  TrocarTexto(comp_tela, "J" + j.toString() + "Horario", hora);
  ...
  importOptionsA.file = new File(baseFolder + 'arquivos/escudos/nba/'+ siglaTimeA + '.png');
  ...
  importOptionsB.file = new File(baseFolder + 'arquivos/escudos/nba/'+ siglaTimeB + '.png');
